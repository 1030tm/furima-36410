...
RSpec.describe User, type: :model do
  ...
  describe 'ユーザー新規登録' do
    ...
    context '新規登録ができないとき' do
      ...
      it 'passwordとpassword_cofirmationが英字のみでは登録できない' do
      ...
      it 'last_kanaが半角では登録できない' do
        ...
        @user.valid?
        expect(@user.errors.full_messages).to include('姓（カナ）は全角文字(カタカナ)を使用してください')
      end
      it 'first_kanaが半角では登録できない' do
        @user.first_kana = 'ｱｲｳｴｵ'
        @user.valid?
        expect(@user.errors.full_messages).to include('名（カナ）は全角文字(カタカナ)を使用してください')
      end
      it 'biryhdayが空だと登録できない' do
        @user.birthday = ''
        @user.valid?
